...
流程：
1. **缺门校验**：若 hand 中含 lackSuit 的牌 → 直接 false。
2. **七对系列判定**（仅当无任何副露 melds）：14 张暗牌恰好分成 7 对则成立；统计 4 张相同的组数 → 龙(1)/双龙(2)/三龙(3)。
3. **标准型判定**：枚举雀头（任一 ≥2 张的 index），移除后对**每门花色独立**递归拆解为顺子/刻子；三门均可拆完且面子总数 = 4 - 副露杠/碰折算数 → 成立。
4. 返回是否可胡 + 该胡法的结构信息（供 fan.ts 用）。
...
### 3.3 番型识别 `fan.ts`
对一手可胡的牌：
1. 判定牌型类别：七对系列 / 对对胡（全刻子+杠）/ 金钩钓（对对胡且除将外全副露单吊）/ 平胡。
2. 判定清一色（全一门）→ 组合成最终 basePattern，查表得 baseFan。
3. 计算加番：
   - **根**：统计 4 张相同组数（按规则：龙七对系列不计根；十八罗汉四杠额外 +4）。
   - 自摸 / 杠上开花 / 海底捞月 / 门清 / 抢杠胡：由 session 传入的上下文标志决定。
4. `totalFan = baseFan + Σ加番`，`cappedFan = min(totalFan, capFan)`，`score = baseScore × 2^cappedFan`。

> 番型番数表硬编码于 `fan.ts` 常量，与 PRD §3.7.2 一致，集中可调。

### 3.4 记分 `scoring.ts`
- **番型付分**：点炮（放炮家一份）/ 自摸（三家各一份），生成 payments，更新 roundScore，并保证零和。
- **杠分（刮风下雨）**：明杠 2×底（放杠家付）；暗杠 2×底×3 家；补杠 1×底×3 家。即时结算，记入 `gangScoreReceived`（含付方，退税用）。
- **抢杠胡**：补杠被抢 → 取消补杠杠分，按点炮胡处理 +1 番。
- **流局查大叫**：计算每个未胡玩家是否听牌（shanten==0）+ 其最大听牌番型分；未听者赔给听牌未胡者。
...
### 4.3 响应优先级
同一张弃牌多家可响应时：**胡 > 杠 > 碰**；多家可胡按座位距打牌者顺时针最近者优先（血战常规）。
